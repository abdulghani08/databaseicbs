...
$query = "SELECT * FROM daftar_pelanggaran WHERE nama LIKE '%$search%' OR jenis LIKE '%$search%' OR klasifikasi LIKE '%$search%' OR poin LIKE '%$search%' LIMIT $limit OFFSET $offset";
$result = mysqli_query($koneksi, $query);

// Query untuk mendapatkan total jumlah data
$total_query = "SELECT COUNT(*) AS total FROM daftar_pelanggaran WHERE nama LIKE '%$search%' OR jenis LIKE '%$search%' OR klasifikasi LIKE '%$search%' OR poin LIKE '%$search%'";
...
<body style="overflow-x: hidden; background-image: url('backgroundgedung.jpg');">
    ...
    <div class="container py-2">
        <h1 class="text-center text-dark" style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;">Data Pelanggaran</h1>
    </div>

    <div class="container mb-6">
        <div class="card py-2" style="overflow-x: auto;">
            <a class="fw-bold py-2" style="font-size: 13px;" href="dt_pelanggaran.php">Lihat Seluruh Table</a>
            <div class="container mt-2 table-responsive text-center ">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-md-8">
                            <div class="input-group input-group-outline">
                                <input type="text" name="search" class="form-control" placeholder="Cari Pelanggaran" value="<?php echo htmlspecialchars($kataPencarian); ?>" />
                            </div>
                        </div>
                        <!-- Kolom tombol -->
                ...
                <a class="btn btn-kuning" href="form_tambahdata_pelanggaran.php">Tambah Nama Pelanggaran</a>
            </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center"><a href="?sort=klasifikasi&order=<?php echo ($sortColumn == 'klasifikasi' && $sortOrder == 'asc') ? 'desc' : 'asc'; ?>">Klasifikasi</a></th>
                                <th class="text-center"><a href="?sort=poin&order=<?php echo ($sortColumn == 'poin' && $sortOrder == 'asc') ? 'desc' : 'asc'; ?>">Poin</a></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                <tr >
                                    ...
                                    <td class="text-center"><?php echo $row['klasifikasi']; ?></td>
                                    <td class="text-center"><?php echo $row['poin']; ?></td>
                                </tr>
                            <?php } ?>
                        </tbody>
                    </table>              
